<template>
	...
	<Form @submit="updateUser" :validation-schema="schema" ref="form" v-show="!cropperOpen">
		...
		<article :class="props.type == 'admin' ? 'row' : ''">
			<!-- Profile section -->
			<section :class="props.type == 'admin' ? 'col-6' : 'col-12'">

				<!-- Photo -->
				<div class="d-flex justify-content-center">

					<button type="button" class="btn btn-photo" @click="openFiles">
						<img :src="photoPreview ?? avatar" alt="Photo" width="150" height="150"
			...
			<!-- Admin Section -->
			<section v-if="props.type == 'admin'" class="col-6 row">

				<!-- Summary -->
				<div class="mb-3 form-group">
					<label for="" class="form-label">My summary</label>
					<Field name="summary" v-slot="{ errorMessage, field }" v-model="userSend.summary">
						<textarea class="form-control" cols="30" rows="4" placeholder="I am a web developer"
				...
				<!-- Coriculum Vitae -->
				<div class="col-12">
					<label for="coriculumVitae" class="form-label">Coriculum Vitae
						<small v-if="userSend.coriculumVitaeURL"> <b>(uploaded)</b></small>
						<small v-else> <b>(empty)</b></small>
					</label>
					<div class="input-group mb-2">
...
const updateAdminInformation = async () => {
	const objectToSend = {
		...
		summary: userSend.value.summary,
		fullName: userSend.value.displayName
	}
